...
# docker as "arch-linux-$arch"
# This script (in contrast to docker upstream on GitHUB) is that it requires
# you to specify $arch so we can create this outside a chroot
# requires root
# requires running script on Arch Linux host

# i686, see: https://wiki.archlinux.org/index.php/Building_32-bit_packages_on_a_64-bit_system
...
case "$arch" in
	...
	x86_64|i686)
		...
		DOCKER_IMAGE_NAME=arch-linux-$arch
		;;

	*)
		# Default to using uname
		arch=$(uname -m)
		export $arch
		PACMAN_CONF='./mkimage-arch-pacman.conf'
		PACMAN_MIRRORLIST='Server = https://mirrors.kernel.org/archlinux/$repo/os/$arch'
...
# Unset and clean exported vars used
unset arch LANG PACMAN_MIRRORLIST
